...
class Hdoctor extends StatefulWidget {
  const Hdoctor({Key? key}) : super(key: key);

  @override
  State<Hdoctor> createState() => _DgeneralState();
}

class _DgeneralState extends State<Hdoctor> {
  final _auth = FirebaseAuth.instance;

  // string for displaying the error Message
  String? errorMessage;
  final _formKey = GlobalKey<FormState>();
  final slmcEditingController = TextEditingController();
    ...
    final searchField = TextFormField(
        ...
        validator: (value) {
          ...
          if (!regex.hasMatch(value)) {
            return ("Enter Valid Doctor ID(Min. 3 Character)");
          }
          return null;
        },
        onSaved: (value) {
...
//slmc field
    final slmcField = TextFormField(
        autofocus: false,
        controller: slmcEditingController,
        validator: (value) {
          RegExp regex = RegExp(r'^.{3,}$');
          if (value!.isEmpty) {
            return ("SLMC cannot be Empty");
          }
          if (!regex.hasMatch(value)) {
            return ("Enter Valid SLMC(Min. 3 Character)");
          }
          return null;
        },
        onSaved: (value) {
          slmcEditingController.text = value!;
        },
        textInputAction: TextInputAction.next,
        decoration: InputDecoration(
